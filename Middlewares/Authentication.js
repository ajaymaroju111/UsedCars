...
const UserRegister = async (req, res) => {
  ...
  try {
    const { profileImage, firstname, middlename, lastname, role, email, password, DOB, phone } =
      req.body;
    if (!email) {
      return res
        .status(401)
...
const getProfile = async (req, res) => {
  try {
    ...
    if(user.status === "inactive"){
      return res.status(401).json({message : "user is not active please verify the email"});
    }
...
const GetProfileById = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ invaid: "User not found" });
    }
